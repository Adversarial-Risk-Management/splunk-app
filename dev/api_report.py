...
script uses it to show the incidents and the risks that Splunk created.
    ...
    python3 dev/api_report.py show INC-00007
    python3 dev/api_report.py show-risk RSK-00003
...
def row_id(identifier):
    """Return the numeric part of a display ID.

    The API paths take the row number, for example 3 for INC-00003. The report
    accepts both forms, because a person reads the display form in the UI.
    """
    digits = re.sub(r"[^0-9]", "", identifier)
    if not digits:
        fail("%s is not a record ID" % identifier)
    return int(digits)
...
def print_description_and_comments(collection, identifier, row, access_token):
    ...
    for line in (row.get("description") or "").splitlines():
        print("    %s" % line)
    comments = get("/v1/%s/%d/comments" % (collection, row_id(identifier)),
                   access_token)
...
def command_show_risk(access_token, arguments):
    if not arguments:
        fail("give a risk ID, for example RSK-00003")
    identifier = arguments[0]
    answer = get("/v1/risks/%d" % row_id(identifier), access_token)
...
def command_show(access_token, arguments):
    if not arguments:
        fail("give an incident ID, for example INC-00007")
    identifier = arguments[0]
    answer = get("/v1/incidents/%d" % row_id(identifier), access_token)
