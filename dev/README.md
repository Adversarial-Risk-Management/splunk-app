...
The API reads a record ID in the display form, `INC-00001` or `RSK-00001`, as
well as the bare number, `1`. The prefix is not case sensitive, and the zeros
in front are optional. Both record types read the same forms, so `show 1` and
`show-risk 1` also work.
